...
for (i in 1:length(branch)) {
  # read in files
  model1 <- read.csv(paste("../DATA/INPUT/", tree, "_locations_", date1, "_branch", branch[i], ".txt", 
                         sep = ""), sep = ",", header = TRUE, skip = 1)
  model2 <-read.csv(paste("../DATA/INPUT/", tree, "_locations_", date2, "_branch", branch[i], ".txt", 
                        sep = ""), sep = ",", header = TRUE, skip = 1)
