...
# calculate L, M, N
dirnum1 <- date2_center.new[1,] - date2_center.new[2,]
dirnum2 <- date2_center.new[3,] - date2_center.new[4,]


# calculate x
x <- (date2_center.new[1,1] * as.numeric(dirnum2[1]) - 
        date2_center.new[3,1] * as.numeric(dirnum1[1])) / 
  (as.numeric(dirnum2[1]) - as.numeric(dirnum1[1]))

# calculate y
y <- (date2_center.new[1,2] * as.numeric(dirnum2[2]) - 
        date2_center.new[3,2] * as.numeric(dirnum1[2])) / 
  (as.numeric(dirnum2[2]) - as.numeric(dirnum1[2]))

# calculate Z
z <- (date2_center.new[1,3] * as.numeric(dirnum2[3]) - 
        date2_center.new[3,3] * as.numeric(dirnum1[3])) / 
  (as.numeric(dirnum2[3]) - as.numeric(dirnum1[3]))
